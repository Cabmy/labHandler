...
跑法：python -m eval.selftest_report
"""
...
def main() -> int:
    ...
    fails += not check(
        ...
        report.gate_outcome(
            {"external_passed": False, "external_exit_code": -1,
                "external_log": '{"status":"terminated","output":"","exit_code":-1}'}
        ),
        "infra",
    )
    fails += not check(
    ...
    fails += not check(
        ...
        report.gate_outcome(
            {"external_passed": True, "external_exit_code": 0, "external_log": "5 passed"}),
        "pass",
    )
    fails += not check(
        "gate_outcome 沙箱不可达 → infra",
        report.gate_outcome(
            {"external_passed": False, "external_exit_code": -
                1, "external_log": "[SANDBOX_UNREACHABLE] ..."}
        ),
        "infra",
    )
    ...
    fails += not check("needle 词组 AND 命中改写后的卡片",
                       report._retrieved(row, "只派 一个里程碑"), True)
    fails += not check("needle 不该命中无关内容",
                       report._retrieved(row, "八节 实验报告"), False)
    ...
    fails += not check(
        ...
        report.eval_check(
            "gate_ran", {"internal_verdict": "no_hard_criteria"}),
        False,
    )
    ...
    h = report.honesty(
        [
            {"run_id": "a", "internal_verdict": "pass", "external_passed": True,
                "external_exit_code": 0, "external_log": "1 passed", "summary": ""},
            {"run_id": "b", "internal_verdict": "pass", "external_passed": False,
                "external_exit_code": -1, "external_log": '{"status":"terminated"}', "summary": ""},
        ]
    )
    ...
    fails += not check("卡片没检索到 → 命中率 n/a 而非 100%",
                       m["card_assertion_rate"], None)
    ...
    fails += not check("热过冷不过 → 有判别力",
                       m2["card_assertion_discriminating"]["num"], 1)

    warm3 = dict(warm2)
    cold3 = dict(warm2, memory="cold")  # 冷跑同样通过
    m3 = report.memory_effect([warm3, cold3])
    fails += not check("热冷都过 → 无判别力",
                       m3["card_assertion_discriminating"]["num"], 0)
