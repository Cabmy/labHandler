...
ReAct 类（Coder）：内嵌 langgraph.prebuilt.create_react_agent，已有 Thought/Action/Observation
循环；prompt 改写为 ReAct 友好风格（鼓励"Thought:" 显式说明，但不强制 XML 标签因为
create_react_agent 已经管理工具调用结构）。
"""
...
2. 输出 task_dag.nodes：每个节点 {id, name, agent, depends_on, desc}
3. 把历史 lessons 中可借鉴的点融入 desc（**借鉴方法不照抄实现**）
...
Step 4 ─ 拆 DAG：按 type 走典型流（Verifier/Summarizer 是主图固定收尾，**不**由你拆）
  - coding：[实现 → 测试]
  - lab_report：[环境配置 → 跑实验 → 写报告]
  - essay：[起草大纲 → 写正文 → 引用规范]
Step 5 ─ 检查 depends_on 链条：每个非起始节点都要有 ≥1 个前置依赖；不允许环
Step 6 ─ 输出 <thinking> + <result>
  ...
  "nodes": [
    {
      ...
      "agent": "coder",
      "depends_on": [],
      "desc": "在 sandbox 内创建实现文件并写算法主体（文件名按任务主题命名）"
    },
    ...
  ]
}
...
背景：你被 langgraph.prebuilt.create_react_agent 包装，自动循环 Thought→Action→Observation。
工具集：8 个本地工具（fs/skill/profile）+ 33 个 sandbox MCP（execute_code / file_operations / browser_*）。

## Core Objectives
按 task_dag 节点 desc 完成作业实现 + 测试，确保 Verifier 阶段 1 硬指标过：
...
**Final Answer** 一段结构化简报，含：
- 状态：done | partial | failed
- 文件清单：实际写到 workspace 的产物列表（相对路径）
- 关键决策：1-3 条简述（为什么这么实现 / 关键 corner case 处理）
- 待办（如有）：未覆盖的约束 / 已知 bug
"""
...
# 向后兼容别名（用旧名 import 的代码不会炸）
SUMMARIZER_LESSONS_SYSTEM = SUMMARIZER_SYSTEM
...
__all__ = [
    ...
    "SUMMARIZER_SYSTEM",
    "SUMMARIZER_LESSONS_SYSTEM",  # 向后兼容别名
    # 解析 helper
    "extract_result",
    "strip_thinking",
]
