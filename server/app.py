"""labHandler Web 服务端（FastAPI + SSE）。

让用户不接触代码即可使用：浏览器上传作业材料（等效放入 workspace/）→
输入任务 → SSE 实时看执行进度 → 下载产物 / 查看 SUMMARY → 归档结束；
另提供 skill 编辑（等效 CLI /edit_skill：提案 → diff 确认 → 落盘）。

架构约定：
- 复用 orchestrator/session.py:TaskSession（与 CLI 同一套会话逻辑，单进程单任务）
- 复用 ui/events.py:iter_graph_events（与终端渲染同一事件流，序列化为 SSE）
- 单任务锁：同一时间只跑一个任务（与单进程单 HwState 架构一致），运行中提交返回 409
- 仅本机使用（默认 127.0.0.1:8000，无鉴权）；如需局域网暴露请自行加反代 + token
...
# .env 集中管理在 config/ 目录（与 cli.py 同路径约定）
...
# SSE 消费队列：每个任务运行期间事件推入此队列；None 哨兵表示流结束
...
def _safe_workspace_path(name: str) -> Path:
    """上传/删除路径防越界：拒隐藏段与绝对路径，resolve 后必须仍在 WORKSPACE_DIR 内。

    保留相对子路径（src/x.py）：删除端点若 basename 化会误指 root 下同名文件。"""
    rel = Path(name)
...
def _list_workspace_files() -> list[dict[str, Any]]:
    ...
    for p in sorted(WORKSPACE_DIR.rglob("*")):
        ...
        if any(part.startswith(".") or part == "__pycache__" for part in rel.parts):
            continue
        out.append({"name": str(rel), "size": p.stat().st_size})
    return out
...
async def download_file(name: str) -> FileResponse:
    # 下载允许一级子目录产物（src/x.py 等）：先按相对路径 resolve 再校验边界
...
async def delete_file(name: str) -> dict[str, Any]:
    """删除单个上传文件：mv 到 .trash/<ts>/（与 clear_workspace 同约定），不真删。"""
    if _is_running():
        raise HTTPException(status_code=409, detail="任务运行中，暂不能修改 workspace")
...
async def _run_graph_task(state: dict[str, Any]) -> None:
    """后台执行主图，把 GraphEvent 序列化推入 SSE 队列。"""
...
async def dream() -> dict[str, Any]:
    ...
    同步 LLM 调用较慢，丢线程池避免阻塞事件循环；任务运行中不允许治理。
    """
    if _is_running():
...
async def remember(req: RememberRequest) -> dict[str, Any]:
    """追加一条长期偏好规则到 preferences.style_rules（等效 CLI /remember，不调 LLM）。"""
...
# pending 提案单槽：单进程单用户，新提案覆盖旧提案；确认须 edit_id 匹配且未超时
...
async def edit_skill(req: EditSkillRequest) -> dict[str, Any]:
    """生成 skill 编辑提案（不落盘）；返回 diff 供前端展示，确认走 /api/edit_skill/apply。"""
...
async def done() -> dict[str, Any]:
    """等效 CLI /done：归档 → 清场 → 强制重建沙箱 → 会话复位（逻辑等效进程重启）。

    复位序列在 orchestrator/session.py:reset（与 CLI 共用）；沙箱重建同步阻塞
    可达几十秒，丢线程池避免卡事件循环。
    """
    if _is_running():
        raise HTTPException(status_code=409, detail="任务运行中，先等它结束")
